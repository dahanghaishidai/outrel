...
<script>
	...
	<#if (d_inde =0 ) >
			 {display : '<#if (dataInfo.COLUMN_COMMENTS??)> <#if (dataInfo.COLUMN_COMMENTS?length>3)>${dataInfo.COLUMN_COMMENTS[0..3]?default("")}<#else>${dataInfo.COLUMN_COMMENTS?default("")}</#if> <#else>${dataInfo.COLUMN_NAME}</#if>', code : '${dataInfo.DTO_COLUMN_NAME?replace("dto.","")}', width : 200,  type:'text'}
			</#if>
	<#if (d_inde >0 ) >
	        ,{display : '<#if (dataInfo.COLUMN_COMMENTS??)> <#if (dataInfo.COLUMN_COMMENTS?length>3)>${dataInfo.COLUMN_COMMENTS[0..3]?default("")}<#else>${dataInfo.COLUMN_COMMENTS?default("")}</#if> <#else>${dataInfo.COLUMN_NAME}</#if>', code : '${dataInfo.DTO_COLUMN_NAME?replace("dto.","")}', width : 200,  type:'text'}
	        </#if>
	...
	<#assign d_inde = dataInfo_index />
			<#if (d_inde = 0 ) >
			 {display : '<#if (dataInfo.COLUMN_COMMENTS??)> <#if (dataInfo.COLUMN_COMMENTS?length>6)>${dataInfo.COLUMN_COMMENTS[0..6]?default("")}<#else>${dataInfo.COLUMN_COMMENTS?default("")}</#if> <#else>${dataInfo.COLUMN_NAME}</#if>', code : '${dataInfo.DTO_COLUMN_NAME?replace("dto.","")}', width : 100, align : 'left', type:'fun', isOrder : false,
			  value:function (obj){
			...
			<#if (d_inde >0 ) >
			,{display : '<#if (dataInfo.COLUMN_COMMENTS??)> <#if (dataInfo.COLUMN_COMMENTS?length>6)>${dataInfo.COLUMN_COMMENTS[0..6]?default("")}<#else>${dataInfo.COLUMN_COMMENTS?default("")}</#if> <#else>${dataInfo.COLUMN_NAME}</#if>', code : '${dataInfo.DTO_COLUMN_NAME?replace("dto.","")}', width : 100, align : 'left', type:'text', isOrder : false}
			</#if>
	</#list>		 
		   ],
