...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/sysMenu/prepareExecute?operateData=toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'isIframe':'false',
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/sysMenu/prepareExecute?operateData=toUpdate&id='+v_ids,
			'width':800,
			'height':500,
			'title':'修改',
			'isIframe':'false',
...
function deleteData(){
	...
	jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		$.ajax({
            type:"POST",
            url:contextRootPath+"/sysMenu/deleteSysMenu?ids="+v_ids,
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/sysMenu/prepareExecute?operateData=toView&id='+vId,
			'width':800,
			'height':500,
			'isIframe':'false',
			'title':'查看明细',
