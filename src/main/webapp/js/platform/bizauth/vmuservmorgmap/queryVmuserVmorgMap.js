...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/vmuserVmorgMap/prepareExecute/toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'buttons':[
...
function doUpdateFrom(){
	...
	var url=contextRootPath+'/vmuserVmorgMap/updateVmuserVmorgMap';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	if(v_ids == ""){
	 ...
	 jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		 $.ajax({
	            type:"POST",
	            url:contextRootPath+"/vmuserVmorgMap/deleteVmuserVmorgMap?ids="+v_ids,
