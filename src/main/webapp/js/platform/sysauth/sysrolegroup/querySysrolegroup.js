...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/sysRoleGroup/prepareExecute/toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'isIframe':'false',
...
function doAddFrom(){
	...
	var url=contextRootPath+'/sysRoleGroup/insertSysRoleGroup';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/sysRoleGroup/prepareExecute/toUpdate?id='+v_ids,
			'width':800,
			'height':500,
			'title':'修改',
			'isIframe':'false',
...
function doUpdateFrom(){
	...
	var url=contextRootPath+'/sysRoleGroup/updateSysRoleGroup';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		$.ajax({
            type:"POST",
            url:contextRootPath+"/sysRoleGroup/deleteSysRoleGroup?ids="+v_ids,
            dataType:'json',
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/sysRoleGroup/prepareExecute/toView?id='+vId,
			'width':800,
			'height':500,
			'title':'查看明细',
			'isIframe':'false',
