...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/sysResource/prepareExecute?operateData=toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'buttons':[
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/sysResource/prepareExecute?operateData=toUpdate&id='+v_ids,
			'width':800,
			'height':500,
			'title':'修改',
			'buttons':[
...
function deleteData(){
	...
	jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		 $.ajax({
	            type:"POST",
	            url:contextRootPath+"/sysResource/deleteSysResource?ids="+v_ids,
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/sysResource/prepareExecute?operateData=toView&id='+vId,
			'width':800,
			'height':500,
			'title':'查看明细',
			'buttons':[
