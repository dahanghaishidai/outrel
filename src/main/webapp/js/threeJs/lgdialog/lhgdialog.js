...
/*! 使用jQ方式调用窗口  与jquery UI 冲突 暂时注释掉 by cg
$.fn.dialog = function()
{
...
{
    return _getMaxZIndex() + lhgdialog.setting.zIndex;
};
...
var  _getMaxZIndex = function() {
		...
		var allTipNum = 0;
		 var allTip = new Array; //存储所有的卡片
     var allTag = document.getElementsByTagName("div"); //根据纸条使用的html标签获取所有的标签以供筛选
     var allTagLen = allTag.length;
 
     for (var i = 0; i < allTagLen; i++) {
         //if (allTag[i].position == 'absolute') {
