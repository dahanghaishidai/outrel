...
(function(){
	var _common = {
		...
		initProInsToDoTurnDirection : function(){
			...
			var v_isJump = $("#isJump").val();
			var v_isBack = $("#isBack").val();
			...
			$.ajax({
				...
				type: 'POST',
				async:false,
				data:{processInsId:v_proInsId,acitityName:v_actNa,isJump:v_isJump,isBack:v_isBack},
				//dataType: 'json',
		...
		getTaskInfo:function(v_myTurn){
			...
			$.ajax({
				url: contextRootPath+'/workFlowProvider/isShowTaskInfo.do',
				type: 'POST',
				async:false,
				data:{processInsId:processInsId,acitityName:acitityName,myTurn:v_myTurn},
			...
			if(("跳转"==v_myTurn&&isJump=="true")||"回退"==v_myTurn&&isBack=="true"||nextFlag){
				
				var v_url = contextRootPath+'/component/jbpm/selectPartner/querySelectTask.jsp?myTurn='
			    +encodeURI(v_myTurn)+'&isAll='+encodeURI(isAll);
				$.dialog({
					id:	'selectTaskInfoDialogId',
				    lock: true,
				    width:500,
				    height:400,
				    title:'选择下一环节',
