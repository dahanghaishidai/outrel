...
SearchForm.prototype = {
	...
	show : function(isShow) {
		debugger;
		var that = this;
		$(that.container).html(that.create());
		that.reDrawObj();
		if (that.checkInit) {
	...
	create : function() {
		...
		debugger;
		var formStrs = [];
		if (typeof this.formStructure === 'object'
				&& $.isEmptyObject(this.formStructure)) {
			...
			if (that.formHTML && that.formHTML.length) {
				if (that.container.tagName === "form") {
					...
					formStrs.push('<form class="searchFrom" isCheck=true id="'
							+ that.formName + '">');
					formStrs.push(that.formHTML);
					$("#" + that.form).remove();
					formStrs.push('</form>');
			...
			formStrs = [ '<form class="searchFrom" isCheck=true id="'
					+ that.formName + '">' ];
	...
	checkFormFormat : function() {
		return submitCheckIsNull(this.formName)
				&& checkFormFormat(this.formName);
	},
	reDrawObj : function() {
		var that = this;
		events = {
			...
			date : function(id) {
				// $(id).datepicker($.datepicker.regional[ "zh-TW"
			...
			dbDateType : function(id, c) {
				var startId = id + "_start";
				var endId = id + "_end";
				var _df_S = c.dateFormatS || 'yyyy-MM-dd';
				var _df_E = c.dateFormatE || 'yyyy-MM-dd';
				var dateObj_S = {
					dateFmt : _df_S
				};
				var dateObj_E = {
			...
			tree : function(id, c) {
				...
				var treeMenuObj = $(id + "_").treeMenu({
					"treeUrl" : c.value,
					"width" : w,
					"height" : h,
					"treeType" : type,
					"disabledLink" : dl,
					"treeExtendObjs" : $(id + "_nodeObj")[0],
					"okFun" : c.okFun,
					"treeIdObj" : $(id)[0]
				});
		...
		for ( var i = 0; i < that.btns.length; i++) {
			...
			// 查找、重置按钮icon转化为字体图标
			if (btn["icon"] == 'ui-icon-search') {
				btn["icon"] = 'icon-search'
			}
			if (btn["icon"] == "ui-icon-extlink") {
				btn["icon"] = 'icon-external-link'
			}
			$("#" + btn["id"]).button({
				icons : {
					primary : btn["icon"]
			...
			if (btn["icon"] == 'icon-search') {
				$("." + btn["icon"]).removeClass().addClass(btn["icon"])
						.parents(".ui-button").addClass("btn-success");
			}
			if (btn["icon"] == 'icon-external-link') {
				$("." + btn["icon"]).removeClass().addClass(btn["icon"])
						.parents(".ui-button").addClass("btn-danger");
			}
		}
	},
	createBtn : function(btnObj) {
		...
		for ( var i = 0; i < this.btns.length; i++) {
			...
			btnStr.push('<button id="' + btn["id"] + '">' + btn["text"]
					+ '</button>');
		}
		btnStr.push("</div>");
		return btnStr.join("");
	...
	getObj : function(code) {
		...
		for ( var i = 0; i < that.columns.length; i++) {
			...
			if (column["code"] === code
					&& (column["type"] === "dbDate" || column["type"] === "dbText")) {
	...
	createObj : function(obj) {
		...
		var objs = {
			select : function() {
				objStrs.push('<select class="fieldSelect" name="' + name
						+ '" id="' + id + '">');
				for ( var i = 0; i < obj.value.length; i++) {
					...
					objStrs.push('<option ' + selected + ' value="' + o.value
							+ '">' + o.text + '</option>');
				}
				objStrs.push('</select>');
			},
			amount : function() {
				dbStyle = 'style= " width:92px; "';
				objStrs.push(createInput("text", startName, startName, value,
						dbStyle));
				objStrs.push("&nbsp;~&nbsp;");
				objStrs.push(createInput("text", endName, endName, value,
						dbStyle));
			},
			date : function() {
			...
			dbDate : function() {
				dbStyle = 'style= " width:92px; "';
				objStrs.push(createInput("text", startName, startId, value,
						dbStyle));
				objStrs.push("&nbsp;~&nbsp;");
				objStrs
						.push(createInput("text", endName, endId, value,
								dbStyle));
			},
			dateTime : function() {
				objStrs.push(createInput("text", name, id, value, dbStyle));
			...
			tree : function() {
				objStrs.push(' <input type="hidden" name="' + name + '" id="'
						+ id + '">');
				objStrs.push(' <input type="hidden" name="' + name
						+ '_nodeObj" id="' + id + '_nodeObj">');
				objStrs.push(createInput("text", name+'_', id+'_', '', dbStyle));
				/*objStrs.push(' <input type="text" name="' + name + '_" id="'
						+ id + '_">&nbsp;');*/
			},
			radio : function(type) {
				var t = type || "radio";
				for ( var i = 0; i < obj.value.length; i++) {
					var o = obj.value[i];
					objStrs.push('<span><input type="' + t + '" id="'
							+ (id + i) + '" name="' + name + '" value="'
							+ o.value + '"/><label for="' + (id + i) + '">'
							+ o.text + '</label></span>')
				}
			},
			checkbox : function() {
			...
			multiautocomplete : function() {
				...
				objStrs.push('<input id="' + id + '_source" name="' + name
						+ '" type="hidden">');
				objStrs
						.push('<select class="fieldSelect" multiple="multiple" id="'
								+ id + '"' + dbStyle + '>');
				for ( var i = 0; i < obj.value.length; i++) {
					var o = obj.value[i];
					var selected = "";
			...
			multiselect : function() {
				objStrs.push(' <input type="hidden" name="' + name
						+ '" id="source_' + name + '">');
				objStrs
						.push('<select multiple="multiple" class="fieldSelect" name="show_'
								+ name
								+ '" id="'
								+ id
		...
		var objStrs = [ ' <div class="field"><label class="fieldName" for="speed">'
				+ (obj["display"] || "&nbsp;") + ':</label>' ];
		objs[obj.type]();
		objStrs.push('</div>');
		return objStrs.join("");
