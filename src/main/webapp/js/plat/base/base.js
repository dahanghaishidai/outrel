...
function jyAjax(url, params, successFun, errorFun, type, Async,obj, isNoShowMask,otherParams) {
	...
	var ajaxObj={
			...
			error : function(XmlHttpRequest, textStatus, errorThrown) {
				mask.close(); 
				if(textStatus !== "abort"){ //异步请求时间过长人为终止请求
				if (errorFun) {
...
Mask.prototype = {
    ...
    show:function(showText){
    	...
    	$(that.container).append(
				'<div style="position:fixed;width:100%;top:0px;bottom:0px;z-index:100000;background:rgba(73, 69, 69, '+tsV+')" id="'
						+ that.maskName
						+ '">'
						+ '<div style="position: absolute;padding-right: 5px;left:50%;'+isDisplay
						+ 'border: 2px solid #69AAE6;border-radius:4px;background:#FFFFFF; margin-left:-55px;top:50%;margin-top:-15px;margin:auto;">'
						+ '<div class="loading"></div><div style="float: left;margin: 6px;font-size:12px;">'+text+'</div></div></div>');
