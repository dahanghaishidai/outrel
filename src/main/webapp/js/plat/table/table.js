...
Table.prototype = {
  ...
  init: function() {
    this.ajaxItem;
    this.isNewPage = this.ts.isNewPage;
    this.pageSize = this.ts.pageSize || 10; //单页显示信息条数
    this.pageIndex = this.ts.pageIndex || 1; //分页显示当前页
    this.totalRows = 0; // 总记录行数
    this.pageCount = 0; // 共有多少页（总页数）
    this.pageRows = 0; // 当前页记录个数
    this.url = this.ts.url || ""; //请求URL
    this.params = this.ts.params || ""; //请求参数
    ...
    this.trSelectedColor = '#ffe681'; //选择框颜色
    ...
    if (document.getElementById('jy-table')) {
      this.setTablePro();
    }
  ...
  changeValByColCode: function(colCode, colVal, rowKey) {
    ...
    if (rowKey) {
      ...
      row = $('#' + that.bTableName + ' tr.trSelected')[0]; //当前选中行tr
    }
      ...
      if (pojo) {
        ...
        for (var extName in pojo) {
          ...
          if (tdColumn) {
            var editObj = {
              "oldValue": "",
              "newValue": pojo[extName],
              "code": extName,
              "name": tdColumn.display,
              "type": "3"
            }
   ...
   */
  show: function(isShow,asyn) {

    var that=this;
    var _varStr = "";
    var _bizTotalFlagVal ="";
    try{
        if (typeof(that.form) === "string") {
            //_varStr = $("#" + that.form).serialize();
        } else if (typeof(that.form) === "object"&&that.form instanceof SearchForm) {
            _bizTotalFlagVal = that.form.getObj("bizTotalFlag").value;
            if((asyn||typeof(asyn)=="undefined" ) && 'Y'== _bizTotalFlagVal){
                that.form.getObj("bizTotalFlag").value='N';
                _bizTotalFlagVal = that.form.getObj("bizTotalFlag").value;
            }
           // _varStr =  that.form.serialize();
        }
    ...
    //重新绘制tbody
    var params = that.getPagesParams() + "&" + that.serializeForm(asyn);
    if(that.params){
    	params+="&"+that.params;
    }
        ...
        that.eachAllTd(function(td) {
          ...
          if (text.trim().length) {
            ...
            if (lsW.width() > tdObj.width() || lsW.height() > tdObj.height()) {
            	if(tdObj.find('a')){
            		var tdObj_ = tdObj.find('div.tdInner');
            		tdObj_.newTooltip({
                        "content": tdObj.text()
                      });
            		tdObj.find('a').mousedown(function(){
            			tdObj_.tooltip('close');
                    });
            	}else{
            		tdObj.newTooltip({
                        "content": tdObj.text()
    ...
    if('N'== _bizTotalFlagVal ){
    	if (typeof(that.form) === "object"&&that.form instanceof SearchForm) {
    	    that.form.getObj("bizTotalFlag").value='Y';
...
/*showEnd*/
  ...
  query: function(params,asynQuery) {
    //查询业务数据
    this.pageIndex = 1;
    this.totalRows=0;
    this.params = params || this.params;
    this.show(true,asynQuery);
  },
  ...
  resize: function() {
    ...
    if(basicObj[0]){
    	var toolH = basicObj[0].offsetTop;
    }else{
    	var toolH = 0;
    }
//  var toolH = basicObj[0].offsetTop;
    ...
    var createHead = function(columns, isLock, tableName, headName, contentTableName) {
      if (columns && columns.length) {
        ...
        for (var i = 0; i < columns.length; i++) {
          ...
          if (c["type"] == "hidden") {
            htr.push('<th headMove="true" style="display:none" ' + linkColumn + linkTable + ' index="' + (tdIndex++) + '"><div headMove="true" class="tdInner" style="width:' + c.width + 'px"><span class="tableHR">' + tc + '</span>' + order + '</div></th>');
          } else {
   ...
   * 2017-06-12
   */
  setTablePro: function() {
    var that = this;
    var table = $("#jy-table");
    var atrributes = table[0].attributes;
    var abs = {};
    $.each(atrributes, function(i, attrib) {
      abs[attrib.name] = attrib.value;
    });
    var options = {};
    ...
    this.isNewPage =abs.isnewpage || this.ts.isNewPage;
    this.pageSize = abs.pagesize || this.pageSize;
    this.pageIndex = abs.pageindex || this.pageIndex;
    this.url = abs.url || this.url;
    this.selectType = abs.selecttype || this.selectType;
    this.isCheck = (abs.ischeck === 'true' ? true : false) || this.isCheck;
    this.hiddenFooter = abs.hiddenfooter || this.hiddenFooter;
    this.rowH = abs.rowh || this.rowh;
    this.isPage = (abs.ispage === 'false' ? false : true) || this.isPage;
    this.form = abs.form || this.form;
    this.rownumbers = (abs.rownumbers === 'true' ? true : false) || this.rownumbers;
    this.primaryKey = abs.primarykey || this.primaryKey;

    this.changePageFun = eval(abs.changepagefun) || this.changePageFun;
    this.dbClickFun = eval(abs.dbclickfun) || this.dbClickFun;
    this.checkAllFun = eval(abs.checkallfun) || this.checkAllFun;
    this.cancelCheckFun = eval(abs.cancelcheckfun) || this.cancelCheckFun;
    this.loadEndFun = eval(abs.loadendfun) || this.loadEndFun;
    this.setTrStyle = eval(abs.settrstyle) || this.setTrStyle;
    this.clickFun = eval(abs.clickfun) || this.clickFun;
    this.checkFun = eval(abs.checkfun) || this.checkFun;
    this.editFilterFun = eval(abs.editfilterfun) || this.editFilterFun;
    this.toolbar = abs.toolbar || this.toolbar; //采用原有的table，toolbar 需要自定义，并且将ID值作为赋值
    this.datas = eval(abs.datas) || this.datas;
    this.params = eval(abs.params) || this.params;
    this.pages = eval(abs.pages) || this.pages;
    that.ts.pages = this.pages;


    this.setColumnsData(); // 获得columns
  },
  ...
  setColumnsData: function() {
    ...
    var columns = [];
    var table = $("#jy-table");
    $.each(table.find('.jy-table-con'), function() {
      var option = {};
      var options = {};
      var atrributes = $(this)[0].attributes;
      var abs = {};
      $.each(atrributes, function(i, attrib) {
        abs[attrib.name] = attrib.value;
      });
      ...
      option.code = abs.code;
      option.display = abs.display || option.code;
      option.width = abs.width || 100;
      option.align = abs.align || $(this).css('text-align') || 'left';
      option.type = abs.type || 'text';
      option.isOrder = (abs.isorder === 'true' ? true : false) || false;
      option.isEdit = abs.isEdit;
      option.value = eval(abs.value);
      option.formate = abs.formate;
      option.fullData = eval(abs.fulldata);
      option.filter = eval(abs.filter);
      option.handle = (abs.handle === 'true' ? true : false) || false;
      columns.push(option);
    });
  ...
  createContent: function(isFlush, groupContentFun,isNextPage) {
    var that = this;
    var createTd = function(column, datas, index, isHandle) {
      ...
      if (column.type == "hidden") {
        return '<td ' + tdClass + ' style="display:none" id="' + column.code + '_' + datas[that.primaryKey] + '" extName="' + column.code + '"  isTd="true" ><div title="' + title + '" isTd="true" ' + isEditStr + ' cellType="' + column.type + '" class="tdInner" style="width:' + column.width + 'px; text-align:' + column["align"] + '">' + value + '</div></td>';
      } else {
    ...
    if (that.datas && isFlush&&isNextPage == undefined) {
      groupData();
      return;
    }
    if (that.url) {
      ...
      if (that.params) {
        params = that.getPagesParams() + "&"; //string
        //params+=JsonToString(that.params);
        params += that.params;
      } else {
        params = that.getPagesParams() + "&" + serializeForm();
      }
      var _bizTotalFlag =false;
            try{
                if (typeof(that.form) === "string") {
                    _varStr = $("#" + that.form).serialize();
                } else if (typeof(that.form) === "object"&&that.form instanceof SearchForm) {
                    _bizTotalFlagVal = that.form.getObj("bizTotalFlag").value;
                    if('Y'== _bizTotalFlagVal){
                        _bizTotalFlag = true;
                        that.form.getObj("bizTotalFlag").value='N';
                    }
                }
            }catch(e){}
            if(that.ajaxItem && (!_bizTotalFlag)&&(isNextPage == undefined)){
            	that.ajaxItem.abort();
	    }
            that.datas= [];
    that.ajaxItem =  jyAjax(that.url, params, function(result) {
    	var _lastToltal = that.totalRows;
        if (that.loadEndFun) {
	  ...
	  if(isNextPage || typeof(isNextPage)=="undefined"){
	      if(that.totalRows == 0 && _lastToltal >0){
	          that.totalRows = _lastToltal;
	      }
	      that.datas = result.data;
	  }else{
	        if(that.totalRows == 0 && _lastToltal >0){
      ...
      },null, null, null,null, _bizTotalFlag);
    } else {
  ...
  getPagesParams: function() {
    ...
    serializeForm: function (isNextPage) {
        var that = this;
        var _varStr = "";
        if (that.form) {
            if (typeof(that.form) === "string") {
        ...
        /*if(isNextPage && _varStr){
            if(_varStr.indexOf("bizTotalFlag=Y") >= 0){
                _varStr = _varStr.replace("bizTotalFlag=Y", "bizTotalFlag=N");
            }
        }*/
        return _varStr;
    },
  ...
  createTableFoot: function() {
    ...
    if (!that.isNewPage) {
      ...
      return {
        "getFooterDiv": function() {
          if (!that.hiddenFooter) {
            return '<div class="tableFooterBox" id="' + that.footerName + '"></div>'
          }
          return "";
        },
        "flushFooter": function() {
          ...
          if (!that.hiddenFooter) {
            var flushPage = function(api){
            	var currentIndex = api.getCurrent();
            	that.pageIndex = currentIndex;
//            	that.show(true,true);
            		that.flush(currentIndex);
            }
            $("#" + that.footerName).pagination({
                pageCount:that.pageCount || that.MAXPAGECOUNT,
                jump:true,
                count:1,
                current:that.pageIndex || 1,
                prevContent:'上一页',
                nextContent:'下一页',
	          ...
	          if(that.totalRows){
                $("#" + that.footerName).append('<span class="totalNum">总计：'+that.totalRows+'</span><span class="totalPage">共：'+(that.pageCount?that.pageCount:that.MAXPAGECOUNT)+'页</span>');
          }else{
        	  $("#" + that.footerName).append('<span class="totalPage">共：'+(that.pageCount?that.pageCount:that.MAXPAGECOUNT)+'页</span>');
          }
        }
        that.addFooterEvent();
      }
      ...
      var getEditObj = function(tdData, tdColumn, trData) {
        ...
        var obj = {
          ...
          "tree": function() {
            ...
            tTextObj.treeMenu({
              "treeUrl": tdColumn.url,
              "treeType": "radio",
              "width": tdColumn.width,
              "height": "auto",
              "isFocus": true
            });
            return tTextObj;
          ...
          if (extName) {
            ...
            if (tdColumn.isEdit) {
              ...
              if (tdColumn.linkObj) {
                ...
                linkDownObj.push([html, {
                  "linkObj": tdColumn.linkObj + "_",
                  type: tdColumn.linkType,
                  linkFun: tdColumn.linkFun,
                  linkUrl: tdColumn.linkUrl,
                  "defaultValue": linkObjValue,
                  "callFun": function(datas) {
