...
<script type="text/javascript">
    ...
    function uploadSuccess(file, serverData) {
		...
		try {
			...
			var upd_Source = $("#upd_Source").val();
			if(upd_Source == "FY"){//是发起客户复议上传的附件
				
				var attTypeCode = $("#attTypeCode").val();//附件类型
				var into_Id = $("#into_Id").val();//进件编号
				
				if(attTypeCode == "L2"){
				}else{//附件不是复议材料
					
				
					//更新复议申请信息表，标明该复议进件的附件已经修改
					var v_url = contextRootPath+"/lbTReconsideration/updateIfAttachModify?intoId="+into_Id;
					jyAjax(v_url,
							'',
							function(msg){
