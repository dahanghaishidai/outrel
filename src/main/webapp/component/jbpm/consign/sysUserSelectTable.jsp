...
function callBackDataInf(){
	...
	//将选中的 角色code 回写至 父页面
	$(window.parent.document).find("#${param.displayUserNameId}").val(v_oldNames);
	$(window.parent.document).find("#${param.hiddenUserId}").val(v_oldIds);
	$(window.parent.document).find("#partnerIframeId").hide();
}
/*
 * 关闭操作
 */
...
<!-- 列表按钮操作 start -->
	<div id="userTableToolbar" class="tableToolbar" style="display:none;">
		  	...
		  	<a href="javascript:void(0)" onclick="doClose()" index="0">关闭</a>
	 </div>
<!-- 列表按钮操作 end -->
