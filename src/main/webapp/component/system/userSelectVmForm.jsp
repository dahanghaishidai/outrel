...
var receiveData = function (datas)
{
	$("#orgName").val(datas[0].NAME);
}
...
function selectUser(){
	//取到元素id
	var dialogStruct={
			'display':contextRootPath+'/component/system/vmSysUserSelect.jsp?showLowerUser=false',
			'width':1200,
			'height':800,
			'title':'选择用户',
			'isIframe':'false',
...
function selectOrg(){//弹出树形选择界面的方法 不做限制，只要能弹出选择框就行
	var dialogStruct={
			'display':contextRootPath+'/component/system/vmTreeSysOrgSelect.jsp',
			'width':800,
			'height':500,
			'title':'选择机构',
			'isIframe':'false',
...
<input name="orgName" id="orgName" onclick="selectOrg();">
