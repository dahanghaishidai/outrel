...
function initFn (){
	//定义 form表单查询 信息
	 var formStructure={
        ...
        ,{display : '机构/部门', code : 'orgDeptName', width : 200,  type:'autocomplete', value:<syscommon:commonTag type="json" codeType="LOAN" beanID="com.jy.modules.platform.sysdict.impl.SysCommonTagAPImpl"></syscommon:commonTag>}
        </c:if>
	...
	if(_orgType !="" && _orgType != 'null' )
		uriParam= uriParam+"&orgType="+_orgType;
	//定义 table 列表信息	
	var tableStructure = {
		//定义table 列表的表头信息
		columns : [
		...
		url : "${basePath}vmuserVmorgMap/queryListVmuserMap"+uriParam,
...
$(document).ready(function(){
	...
	//var orgurl=contextRootPath+'/vmtreeInfo/queryTreeVMOrgByOrgType?orgType='+orgtype+'&orgId=';
   	var orgurl=contextRootPath + '/vmtreeInfo/queryVmTreeSysOrg?_orgId=' + orgId + '&orgType='+orgtype;		
	$("[name='orgDeptName']").next(".custom-combobox").find("a").treeMenu(
   			{"treeUrl":orgurl
   			,"treeType":"radio","disabledLink":"true"
	   		,"treeIdObj":$("[name='orgId']")
	   		,"treeNameObj":$("[name='orgDeptName']").next(".custom-combobox").find("input")
	   		,"width":"200"
	   		,"height":"300"});
...
<input type="hidden" id="orgType" name="orgType" value="${param.orgType}"/>
