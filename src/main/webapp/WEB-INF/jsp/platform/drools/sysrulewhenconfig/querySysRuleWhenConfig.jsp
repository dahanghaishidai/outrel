...
<!-- 列表按钮操作 start -->
	<div id="tableToolbar" class="tableToolbar" style="display:none;">
		  <shiro:hasPermission name="platform/drools/sysrulewhenconfig/querySysRuleWhenConfig:add">
		  	<a href="javascript:void(0)" onclick="toAddData()" index="0">新增</a>
		  </shiro:hasPermission>
	  	 <shiro:hasPermission name="platform/drools/sysrulewhenconfig/querySysRuleWhenConfig:modify">
		  	<a href="javascript:void(0)" onclick="toUpdateData()" index="1">修改</a>
		  </shiro:hasPermission>
		  <shiro:hasPermission name="platform/drools/sysrulewhenconfig/querySysRuleWhenConfig:delete">
	...
	function initFn(){
		//定义 form表单查询 信息
		 var formStructure={
	        ...
	        ,{display : ' 中文规则 ', code : 'ruleNameCh', width : 200,  type:'text'}
	        ,{display : ' 英文规则 ', code : 'ruleNameEn', width : 200,  type:'text'}
	        ...
	        ,{display : ' 比较值 ', code : 'compareValueEn', width : 200,  type:'text'}
	        ,{display : ' 比较值中 ', code : 'compareValueCh', width : 200,  type:'text'}
	        ,{display : ' 后括号 ', code : 'afterBrackets', width : 200,  type:'text'}
	        ,{display : ' 逻辑操作 ', code : 'logicalOperator', width : 200,  type:'text'}
	        ,{display : ' 手输规则 ', code : 'manualRule', width : 200,  type:'text'}
		...
		var tableStructure = {
			//定义table 列表的表头信息
			columns : [
			 {display : ' 编码 ', code : 'ruleCode', width : 100, align : 'left', type:'fun', isOrder : false,
			  value:function (obj){
						return "<a href='javascript:void(0)' onclick='viewData("+obj.id+")'>"+obj.ruleCode+"</a>";
				    }	 
			 }
			,{display : ' 中文规则名 ', code : 'ruleNameCh', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 英文规则名 ', code : 'ruleNameEn', width : 100, align : 'left', type:'text', isOrder : false}
			...
			,{display : ' 比较值 ', code : 'compareValueEn', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 比较值中文 ', code : 'compareValueCh', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 后括号 ', code : 'afterBrackets', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 逻辑操作符 ', code : 'logicalOperator', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 手输规则 ', code : 'manualRule', width : 100, align : 'left', type:'text', isOrder : false}
			...
			url : "${basePath}sysRuleWhenConfig/queryListSysRuleWhenConfig",
