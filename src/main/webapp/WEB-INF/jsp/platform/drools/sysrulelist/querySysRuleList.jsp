...
<head>
  ...
  <script type="text/javascript" src="${basePath}/js/platform/droolssysrulelist/querySysRuleList.js"></script>
<!-- 相关js方法 -->
...
<!-- 列表按钮操作 start -->
	<div id="tableToolbar" class="tableToolbar" style="display:none;">
		  <shiro:hasPermission name="platform/droolssysrulelist/querySysRuleList:add">
		  	<a href="javascript:void(0)" onclick="toAddData()" index="0">新增</a>
		  </shiro:hasPermission>
	  	 <shiro:hasPermission name="platform/droolssysrulelist/querySysRuleList:modify">
		  	<a href="javascript:void(0)" onclick="toUpdateData()" index="1">修改</a>
		  </shiro:hasPermission>
		  <shiro:hasPermission name="platform/droolssysrulelist/querySysRuleList:delete">
	...
	function initFn(){
		//定义 form表单查询 信息
		 var formStructure={
			// 定义form表单 字段信息
			columns : [
			 {display : ' 类型：1 ', code : 'ruleType', width : 200,  type:'text'}
	        ,{display : ' 中文名 ', code : 'chName', width : 200,  type:'text'}
	        ,{display : ' 英文名： ', code : 'enName', width : 200,  type:'text'}
	        ,{display : ' 生效时间 ', code : 'eftectTime', width : 200,  type:'text'}
		...
		var tableStructure = {
			//定义table 列表的表头信息
			columns : [
			 {display : ' 类型：1 规则 ', code : 'ruleType', width : 100, align : 'left', type:'fun', isOrder : false,
			  value:function (obj){
						return "<a href='javascript:void(0)' onclick='viewData("+obj.id+")'>"+obj.ruleType+"</a>";
				    }	 
			 }
			,{display : ' 中文名 ', code : 'chName', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 英文名：如果是 ', code : 'enName', width : 100, align : 'left', type:'text', isOrder : false}
			,{display : ' 生效时间，只有 ', code : 'eftectTime', width : 100, align : 'left', type:'text', isOrder : false}
			...
			url : "${basePath}sysRuleList/queryListSysRuleList",
