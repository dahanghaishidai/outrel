...
<%-- <link rel="stylesheet" href="<%=basePath%>css-blue/home/home.css" /> --%>
<style>
	.them-box{ position:relative; cursor:pointer;}
	.them-box:hover .them-con{		display:block;	}
	.them-box .them-con{		display: none;	   font-size:14px; cursor:default; position: absolute;	    width: 100px;	    height: 100px;	    background: #fff;	    z-index: 99;	    left: -36px;	    border: 1px solid #bdc9c9;	    border-radius: 5px;	    padding: 15px 10px;	}
	.them-box .them-item {    display: inline-block;  cursor:pointer;  width: 30px;    height: 30px;    border-radius: 30px;margin: 0px 3px;}
	.them-box .them-item:active{ border:1px solid #eee; border-radius:30px;width:28px;height:28px;}
	.them-box .blue{ background:#26b3c0;}
	.them-box .black{ background:#272930;}
	.them-box .yellow{ background:#005AAB;}
	.them-box .gray{ background:#C1C1C1;}
</style>
<script type="text/javascript">
...
<body>
   ...
   <%--  <li><a href="<%=ObtainPropertiesInfo.getValByKey("cas.logout.url") %>?service=<%=basePath%>user/logout">退出</a></li> --%>	
    <li><a class="icon exit" href="<%=ObtainPropertiesInfo.getValByKey("cas.logout.url") %>" title="退出"><span class="icon-off"></span></a></li><!-- <li class="liSplit"></li> -->
    <li class="them-box icon">
    	<span class="icon-th-large"></span>
	    <div class="them-con" id="them-con">
	        <span class="black them-item" id="black"></span>
	        <span class="blue them-item" id="default"></span>
	        <span class="yellow them-item" id="yellow"></span>
	        <span class="gray them-item" id="gray"></span>
	    </div>
	</li>
    <li><a  class="icon setting" href="<%=ObtainPropertiesInfo.getValByKey("cas.modifypwd.url") %>?username=<shiro:principal property="loginName"/>" target="_blank" title="修改密码"><span class="icon-cog"></span></a></li>
  <!--   <li><a href="javascript:;" onclick="openQuestionWindow()">IT支持</a></li> -->
    <li><a class="icon msg" href="#" onclick="triggerMenu('/sysMessagecenter/prepareExecute/toQueryPage')" title="消息"><span class="icon-bell iconfont20"></span>(<span id="user_message_count"></span>)</a></li>
 	<li class="liSplit"></li> 
	...
	window.onload=function(){
		...
		jyAjax("<app:contextPath/>/sysMenu/querySysMenuForHome","&sortName=id&sortType=asc",function(result){
			var v_boolean = false;
// 			v_boolean = ckSignIsLimit(v_boolean);
			var maps={};
			var datas=[];
			...
			if(obj.id=="shrinkIcon"){
				if(endObj.hasClass("shrinkIconLeft")){
					...
					leftObj.css("width","0px");
		       	 	mainObj.css("left",(menuL+moveW)+"px");
		       	 	moveLeft.css("left",(menuL)+"px");
				}			
			}
		});
	...
	(function(){
		var mainTabs=[{"title":"工作台","url":"","isLock":true}];
	...
	(function(){
		...
		if("YES" =="<%=acceptMsg%>"){
			...
			var msgStructure={"waitTime":"60000","title":"系统新消息","type":"message","url":url,"params":{systemFlag:sys},"msgFun":function(){
		 	...
		 	//加入pushlet会话并监听
			//为当前用户发布一条欢迎消息
// 			var url='<app:contextPath/>/home/queryRoleByUserNo?userNo='+${userInfo.userNo};
			//通过ajax保存
...
// 		  	});
	 ...
	 * 签约用户签约用户是否被锁定
	 */
	function ckSignIsLimit(v_boolean){ 
